...
const ConvertedLead = () => {
    ...
    useEffect(() => {
        ...
        getAllLeads(headers)
            .then((response) => {
                const convertedLead = response.data.Leads.filter((item) => {
                    return item.status === "Converted"
                })
                setLeadList(convertedLead);
                setLoader(false);
            })
            .catch((error) => {
                console.log(error);
            ...
            <div className="d-flex align-items-start justify-content-between">
                ...
                <div className="d-flex justify-content-end">
                    <button className='add-student-button me-1'>
                        <CSVLink data={leadList} headers={headers1} filename='ConvertedLeads_Records.csv'
